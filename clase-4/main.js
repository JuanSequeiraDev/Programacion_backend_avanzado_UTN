"use strict";
/* Objeto literal*/
/* interface Producto { //Palabra reservada interface
    nombre: string,
    precio: number,
    id: number,
    categoria: string,
    stock: number,
    descripcion: string
}

const producto: Producto = {
    nombre: 'Tv samsung',
    precio: 2000,
    id: 1,
    categoria: 'TECNOLOGIA',
    stock: 10,
    descripcion: 'lorem'
}

const producto_2: Producto = {
    nombre: 'Tv samsung',
    precio: 2000,
    id: 1,
    categoria: 'TECNOLOGIA',
    stock: 25,
    descripcion: 'lorem'
}




//Creando una propiedad
producto.stock = 10

//Reasignar propiedad
producto.nombre = 'LG 50"'


const venderProducto = (producto: Producto) => {
    console.log("Has vendido a " + producto.nombre + 'y ahora me quedan ' + producto.stock)
}

venderProducto(producto)
venderProducto(producto_2)



const mixed : [number, string, boolean] = [10, 'pepe', false]
const nombres : string[] = ['pepe', 'juan', 'maria']
const notas : number[] = [10, 7, 5]

const productos : Producto[] =[
    producto,
    producto_2
]
 */
let id_counter = 0;
class Usuario {
    //TIPADO DE LOS PARAMETROS DE LA FUNCION CONSTRUCTORA
    constructor(nombre, role, clave, email, edad, cuit, id) {
        this.nombre = nombre;
        this.role = role;
        this.clave = clave;
        this.email = email;
        this.edad = edad;
        this.cuit = cuit;
        this.id = id;
    }
}
/* const usuario_1 : Usuario = new Usuario('pepe', 'admin', '123', 'pepe@gmail.com', 72, 22132126)
const usuario_2 : Usuario = new Usuario('pepe', 'admin', '123', 'pepe@gmail.com', 72, 22132126) */
class ManejadorUsuarios {
    constructor() {
        this.usuarios = [];
        this.id_counter = 0;
    }
    agregarUsuario(nombre, role, clave, email, edad, cuit) {
        /* const {nombre, role, clave, email, edad, cuit} */
        const nuevo_usuario = new Usuario(nombre, role, clave, email, edad, cuit, this.id_counter++);
        this.usuarios.push(nuevo_usuario);
    }
}
const manejador_usuarios = new ManejadorUsuarios();
manejador_usuarios.agregarUsuario('pepe', 'admin', '123', 'pepe@gmail.com', 53, 22132126);
console.log(manejador_usuarios);
/* console.log(usuario_1, usuario_2) */
