...
productsRouter.delete('/:product_id', async(req, res)=>{
    try {
        ...
        const products = JSON.parse(await filesystem.promises.readFile('./data/products.json', { encoding: 'utf-8' }))
        const productFound = products.find(product => product.id == product_id)
        ...
        if (productFound === undefined) {
            const responseFalse = new ResponseBuilder()
                ...
                .setMessage("No se encontro el producto")
                .setStatus(404)
                .setData({ message: 'No se encontro el producto'})
                .build()
            return res.status(404).json(responseFalse)
        }

        const productFoundIndex = products.findIndex(product => product.id == product_id)
        products[productFoundIndex].active = false
...
import {getAllProductsController, getProductByIdController, postProductController, putProductController} from "../products.controllers.js"
